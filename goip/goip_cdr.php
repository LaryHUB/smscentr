...
$sql="SELECT goipid,sum(expiry) as calltime,count(record.id) as callcount,count(goipid) from record left join goip on goip.id=record.goipid where dir=2 and expiry>0 and time>'$start_time' and time<'$end_time' $where group by goipid";
